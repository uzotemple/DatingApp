...
class _RelationshipScreenState extends State<RelationshipScreen> {
  ...
  int selectedIndex = 0; // Default to 'Single'
  ...
  void initState() {
    ...
    // Fetch the saved relationship status
    String? currentStatus = profileProvider.getProfileData?['relationshipStatus'];

    if (currentStatus != null && items.contains(currentStatus)) {
      selectedIndex = items.indexOf(currentStatus);
    }
  }
  ...
  void _updateProfile() {
    ...
    // Update profile with selected relationship status
    profileProvider.updateProfile(
      context,
      authProvider,
      {"relationshipStatus": items[selectedIndex]},
    );
  }
      ...
      body: Column(
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
              const Text('(4/11)'), // Step number placeholder
              InkWell(
                onTap: () {
                  Navigator.push(
                    context,

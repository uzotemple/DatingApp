...
class AuthProvider with ChangeNotifier {
  final _secureStorage = const FlutterSecureStorage(); // Secure storage
  String? _accessToken;
  String? _refreshToken;
  ...
  Future<void> loadTokens() async {
    _accessToken = await _secureStorage.read(key: 'accessToken');
    _refreshToken = await _secureStorage.read(key: 'refreshToken');
    _userId = await _secureStorage.read(key: 'userId'); // Load userId
    notifyListeners();
  }
  ...
  Future<void> refreshToken() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final newAccessToken = responseBody['accessToken'];
        final newRefreshToken = _extractRefreshToken(response.headers);
        final newUserId = responseBody['userId']?.toString(); // Extract userId if available

        if (newAccessToken != null && newRefreshToken != null && newUserId != null) {
          await storeTokens(newAccessToken, newRefreshToken, newUserId);
          developer.log('Access token refreshed successfully');
        } else {
          throw Exception('Failed to extract tokens from refresh response');
        }
  ...
  Future<void> logout() async {
    await _secureStorage.delete(key: 'accessToken');
    await _secureStorage.delete(key: 'refreshToken');
    await _secureStorage.delete(key: 'userId'); // Clear userId
  ...
  String? _extractRefreshToken(Map<String, String> headers) {
    final cookies = headers['set-cookie'];
    if (cookies != null) {
      final refreshToken = RegExp(r'local.bg-refresh-mgmt=([^;]+)').firstMatch(cookies);
      return refreshToken?.group(1);
    }
    return null;
  }
